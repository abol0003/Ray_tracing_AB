...
def calculer_angle_incidence(pos_emetteur, pos_incidence, obstacle):
    ...
    """
    vecteur_obstacle = np.array([obstacle.end.x - obstacle.start.x, obstacle.end.y - obstacle.start.y])
    normal_obstacle = np.array([-vecteur_obstacle[1], vecteur_obstacle[0]])
    vecteur_incident = np.array([pos_incidence.x - pos_emetteur.x, pos_incidence.y - pos_emetteur.y])
    normal_obstacle = normal_obstacle / np.linalg.norm(normal_obstacle)
    vecteur_incident = vecteur_incident / np.linalg.norm(vecteur_incident)
    # produit scalaire entre le vecteur incident et la normale à l'obstacle
    cos_theta_i = np.dot(vecteur_incident, normal_obstacle)
