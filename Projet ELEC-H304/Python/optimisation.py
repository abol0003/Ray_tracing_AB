...
def calc_power_at_position(position, env, ray_tracer, width, height):
    ...
    env.emitters = [Emitter(Position(*position), power=20, frequency=60e9)]
...
def eval_emitter_config(width, height, pos1, pos2, cache):
    ...
    Utilise un cache pour stocker et récupérer les résultats des configurations déjà calculées pour minimiser le temps.
    """
    sorted_positions = tuple(sorted((pos1, pos2)))
    if sorted_positions in cache:
        return sorted_positions + (cache[sorted_positions],)
    env = Environment()
    env.emitters = [Emitter(Position(*pos1), power=20, frequency=60e9),
                    Emitter(Position(*pos2), power=20, frequency=60e9)]
...
if __name__ == '__main__':
    ...
    best_position, best_power = optimize_emitter_position_parallel(env, 15.5, 9, 2, 0.5)
    end_time = time.time()
    print(f"Optimal Emitter Position: {best_position} with power: {best_power} dBm")
    print(f"Optimization Time: {end_time - start_time:.2f} seconds")
    start_time2 = time.time()
    best_positions, best_power = optimize_two_emitters_positions_parallel(env, 15.5, 9, 2 , 0.5)
    end_time2 = time.time()
